  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data: authData, error: authError } = await supabase.auth.signUp({
        ...
        options: { data: { full_name: form.full_name, phone: form.phone, role: 'user' } },
      })
      if (authError) { setError(authError.message.includes('already registered') ? 'Этот email уже зарегистрирован. Войдите.' : authError.message); setLoading(false); return }
      if (!authData.user) { setError('Ошибка регистрации'); setLoading(false); return }

      const { error: profileError } = await supabase.from('users').insert({ auth_id: authData.user.id, full_name: form.full_name, phone: form.phone, email: form.email, role: 'user' })
      if (profileError) { await supabase.auth.signOut(); setError('Пользователь с таким email или телефоном уже существует'); setLoading(false); return }
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col justify-center px-6 py-8">
      <div className="max-w-md mx-auto w-full">
        <div className="text-center mb-8">
          ...
          <h1 className="text-xl font-bold text-gray-900">Регистрация покупателя</h1>
          <p className="text-gray-500 text-sm mt-1">Быстрая доставка лекарств</p>
        </div>

        {/* Role switcher */}
        <div className="bg-white rounded-2xl p-1.5 shadow-sm mb-6 flex gap-1">
          <span className="flex-1 text-center bg-blue-600 text-white py-2.5 rounded-xl text-sm font-semibold">👤 Покупатель</span>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm">
          <form onSubmit={handleSubmit} className="space-y-4">
            {[
              { key:'full_name', label:'Полное имя *',  placeholder:'Иван Иванов',       type:'text' },
