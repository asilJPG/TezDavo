"use client";
// src/app/(auth)/login-courier/page.tsx
import { useState } from "react";
import Link from "next/link";
import { createClient } from "@/lib/supabase";

export default function LoginCourierPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (!user || user.role !== "courier") {
      ...
      setError("Этот аккаунт не является курьером");
      setLoading(false);
      return;
    }
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col justify-center px-6 py-8">
      <div className="max-w-md mx-auto w-full">
        <div className="text-center mb-8">
          ...
          <h1 className="text-xl font-bold text-gray-900">Вход для курьеров</h1>
          <p className="text-gray-500 text-sm mt-1">Кабинет курьера</p>
        </div>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm">
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-orange-500 text-white py-3.5 rounded-xl font-bold text-base disabled:opacity-50"
            >
              {loading ? "Входим..." : "Войти в кабинет курьера"}
            </button>
          </form>
