...
export async function POST(req: NextRequest) {
  ...
  const { pharmacy_id, items, delivery_address, delivery_lat, delivery_lng, notes } = body
  ...
  const inventoryIds = items.map((i: { inventory_id: string }) => i.inventory_id)
  ...
  const orderItems = items.map((item: { inventory_id: string; quantity: number }) => {
