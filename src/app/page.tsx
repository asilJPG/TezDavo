      ...
      <div className="max-w-4xl mx-auto px-4 lg:px-8 py-6">
        ...
        <div className="bg-gradient-to-r from-blue-600 to-blue-700 rounded-2xl p-6 lg:p-10 mb-8">
          ...
          <p className="text-blue-200 mb-6 text-sm lg:text-base">
            Toshkent aptekalaridan 30–60 daqiqada
          </p>
          <form
            onSubmit={handleSearch}
            className="flex gap-2 lg:gap-3 max-w-xl"
          >
            <input
              type="text"
              value={search}
            ...
            <button
              type="submit"
              className="bg-white text-blue-600 font-bold px-5 lg:px-6 py-3 rounded-xl whitespace-nowrap text-sm lg:text-base"
            >
              Qidirish
            </button>
          </form>
          <div className="flex gap-2 mt-4 flex-wrap">
            {["Paracetamol", "Ibuprofen", "No-Shpa", "Omeprazol"].map((tag) => (
              <button
                ...
                className="bg-blue-500 hover:bg-blue-400 text-white text-xs lg:text-sm px-3 py-1.5 rounded-full transition-colors"
              >
                {tag}
              </button>
            ))}
        ...
        <div className="grid grid-cols-3 gap-3 lg:gap-5 mb-8">
          {[
            { icon: "⚡", t: "Tez yetkazish", s: "30–60 daqiqa" },
            { icon: "💰", t: "Narxlarni solishtirish", s: "Eng arzon narx" },
            { icon: "🏪", t: "Ko'p aptekalar", s: "Butun Toshkent" },
          ].map((f) => (
            <div
              key={f.t}
              className="bg-white rounded-2xl p-4 lg:p-5 shadow-sm flex items-center gap-3 lg:gap-4"
            >
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 lg:gap-4 mb-8">
          {loading
            ...
            : popular.map((med) => (
                ...
                >
                  <div className="bg-blue-50 rounded-lg h-16 lg:h-20 flex items-center justify-center mb-2 lg:mb-3">
                    <span className="text-3xl lg:text-4xl">💊</span>
                  </div>
                  <div className="font-semibold text-gray-900 text-sm truncate mb-0.5">
                    {med.name}
                  </div>
                  <div className="text-xs text-gray-400 mb-1 lg:mb-2">
                    {med.category}
                  </div>
                  {med.min_price && (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <Link
            href="/ai-chat"
            className="flex items-center gap-3 bg-indigo-50 border border-indigo-100 rounded-2xl p-4"
          >
          ...
          >
            ...
            <div className="flex-1">
              ...
              <div className="text-xs text-gray-500">Eslatmalar va jadval</div>
            </div>
